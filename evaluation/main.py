...
def main_plot(cfg):
    ...
    for dataset in dataset_names:
        ...
        for method in method_names:
            iRes = loadmat(os.path.join(
                cfg.output_dir, method, 'final', dataset + '.mat'))
...
def main(cfg):
    ...
    # model -> ckpt -> dataset
    for method in method_names:
        epochs = os.listdir(os.path.join(
            cfg.pred_dir, method))[-num_model_eval:][::-1]
        for epoch in epochs:
            continue_eval = True
            for dataset in dataset_names:
                loader = EvalDataset(
                    os.path.join(cfg.pred_dir, method, epoch,
                                 dataset),        # preds
                    os.path.join(cfg.gt_dir, dataset)                   # GT
                )
                print('Evaluating predictions from {}'.format(
                    os.path.join(cfg.pred_dir, method, epoch, dataset)))
                thread = Eval_thread(
                    loader, method, dataset, cfg.output_dir, epoch, cfg.cuda)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gt_dir', type=str,
                        default='/gemini/data-1/sod/gts', help='GT')
    parser.add_argument('--pred_dir', type=str,
                        default='/gemini/output/preds', help='predictions')
    parser.add_argument('--output_dir', type=str,
                        default='/gemini/output/output/details', help='saving measurements here.')
    parser.add_argument('--output_figure', type=str,
                        default='/gemini/output/output/figures', help='saving figures here.')
