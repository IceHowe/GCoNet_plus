...
os.chdir('/gemini/code/GCoNet_plus/evaluation')

move_best_results_here = False

record = ['dataset', 'ckpt', 'Emax',
          'Smeasure', 'Fmax', 'MAE', 'Emean', 'Fmean']
measurement = 'Emax'
score_idx = record.index(measurement)

with open('/gemini/output/output/details/result.txt', 'r') as f:
    res = f.read()
...
ss = sorted(score, key=lambda x: (x[record.index('dataset')], x[record.index('Emax')], x[record.index(
    'Smeasure')], x[record.index('Fmax')], x[record.index('ckpt')]), reverse=True)
ss_ar = np.array(ss)
...
for ckpt in ckpts:
    ...
    # Smeasure
    score_coca = float(scores[scores[:, 0] == 'CoCA']
                       [0][record.index('Smeasure')])
    score_cosod = float(scores[scores[:, 0] == 'CoSOD3k']
                        [0][record.index('Smeasure')])
    score_cosal = float(
        scores[scores[:, 0] == 'CoSal2015'][0][record.index('Smeasure')])
    improvements_Smeasure = [
        (score_coca - gco_scores_Smeasure['CoCA']
         ) / gco_scores_Smeasure['CoCA'],
        (score_cosod - gco_scores_Smeasure['CoSOD3k']
         ) / gco_scores_Smeasure['CoSOD3k'],
        (score_cosal -
         gco_scores_Smeasure['CoSal2015']) / gco_scores_Smeasure['CoSal2015']
    ]
...
if best_measurement == 'Emax':
    ...
    best_improvement_index = np.argsort(
        improvements_mean_Smeasure).tolist()[-1]
    best_ckpt = ckpts[best_improvement_index]
    best_improvement_mean_Smeasure = improvements_mean_Smeasure[best_improvement_index]
    best_improvements_Smeasure = improvements_lst_Smeasure[best_improvement_index]
...
print('Got Emax improvements on CoCA-{:.3f}%, CoSOD3k-{:.3f}%, CoSal2015-{:.3f}%, mean_improvement: {:.3f}%.'.format(
    best_improvements[0]*100, best_improvements[1] *
    100, best_improvements[2]*100, best_improvement_mean*100
))
print('Got Smes improvements on CoCA-{:.3f}%, CoSOD3k-{:.3f}%, CoSal2015-{:.3f}%, mean_improvement: {:.3f}%.'.format(
    best_improvements_Smeasure[0]*100, best_improvements_Smeasure[1] *
    100, best_improvements_Smeasure[2]*100, best_improvement_mean_Smeasure*100
))
...
if move_best_results_here:
    ...
    dst = '-'.join((trial, ep))
    shutil.move(os.path.join('/gemini/data-1/sod/preds', dr), dst)
