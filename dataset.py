...
class CoData(data.Dataset):
    def __init__(self, image_root, label_root, image_size, max_num, is_train):
        ...
        self.data_size = (self.size_train, self.size_train) if is_train else (
            self.size_test, self.size_test)
        self.image_dirs = list(
            map(lambda x: os.path.join(image_root, x), class_list))
        self.label_dirs = list(
            map(lambda x: os.path.join(label_root, x), class_list))
    ...
    def __getitem__(self, item):
        ...
        num = len(names)
        image_paths = list(
            map(lambda x: os.path.join(self.image_dirs[item], x), names))
        label_paths = list(map(lambda x: os.path.join(
            self.label_dirs[item], x[:-4]+'.png'), names))
        ...
        if self.is_train:
            ...
            other_image_paths = list(map(lambda x: os.path.join(
                self.image_dirs[other_item], x), other_names))
            other_label_paths = list(map(lambda x: os.path.join(
                self.label_dirs[other_item], x[:-4]+'.png'), other_names))
            ...
            new_image_paths = new_image_paths + \
                [other_image_paths[i] for i in other_sampled_list]
            image_paths = new_image_paths
            new_label_paths = new_label_paths + \
                [other_label_paths[i] for i in other_sampled_list]
        ...
        images = torch.Tensor(
            final_num, 3, self.data_size[1], self.data_size[0])
        labels = torch.Tensor(
            final_num, 1, self.data_size[1], self.data_size[0])
        ...
        for idx in range(final_num):
            if self.load_all:
                ...
                if not os.path.exists(image_paths[idx]):
                    image_paths[idx] = image_paths[idx].replace(
                        '.jpg', '.png') if image_paths[idx][-4:] == '.jpg' else image_paths[idx].replace('.png', '.jpg')
                image = Image.open(image_paths[idx]).convert('RGB')
                if not os.path.exists(label_paths[idx]):
                    label_paths[idx] = label_paths[idx].replace(
                        '.jpg', '.png') if label_paths[idx][-4:] == '.jpg' else label_paths[idx].replace('.png', '.jpg')
                label = Image.open(label_paths[idx]).convert('L')

            subpaths.append(os.path.join(image_paths[idx].split(
                '/')[-2], image_paths[idx].split('/')[-1][:-4]+'.png'))
            ...
            image, label = self.transform_image(
                image), self.transform_label(label)

            images[idx] = image
            labels[idx] = label

        if self.is_train:
            cls_ls = [item] * (final_num // 2) + \
                [other_item] * (final_num // 2)
