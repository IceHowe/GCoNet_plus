...
class Config():
    def __init__(self) -> None:
        ...
        # Augmentation
        self.preproc_methods = ['flip', 'enhance',
                                'rotate', 'crop', 'pepper'][:3]
        ...
        # Refiner
        # 0 -- no refinement, 1 -- only output mask for refinement, 4 -- but also raw input.
        self.refine = [0, 1, 4][0]
        if self.refine:
            self.batch_size = 16
        else:
        ...
        if 'keep in range':
            self.loss_sal_layers = min(
                self.output_number, self.loss_sal_layers)
            self.loss_cls_mask_last_layers = min(
                self.output_number, self.loss_cls_mask_last_layers)
            self.output_number = min(self.output_number, max(
                self.loss_sal_layers, self.loss_cls_mask_last_layers))
        ...
        for loss_sal in self.lambdas_sal_last.keys():
            loss_sal_ratio_by_last_layers = 4 / \
                (int(
                    bool(self.lambdas_sal_others[loss_sal])) * (self.loss_sal_layers - 1) + 1)
        ...
        self.rand_seed = 7
        run_sh_file = [f for f in os.listdir('.') if 'gco' in f and '.sh' in f] + [
            os.path.join('..', f) for f in os.listdir('..') if 'gco' in f and '.sh' in f]
        with open(run_sh_file[0], 'r') as f:
            self.val_last = int(
                [l.strip() for l in f.readlines() if 'val_last=' in l][0].split('=')[-1])
