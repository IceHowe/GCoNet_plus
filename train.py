...
for testset in args.testsets.split('+'):
    test_loader = get_loader(
        os.path.join('/gemini/data-1/sod', 'images',
                     testset), os.path.join('/gemini/data-1/sod', 'gts', testset),
...
if config.lambda_adv:
    ...
    Tensor = torch.cuda.FloatTensor if (
        True if torch.cuda.is_available() else False) else torch.FloatTensor
...
all_params = [{'params': base_params}, {
    'params': model.bb.parameters(), 'lr': config.lr * 0.01}]

# Setting optimizer
optimizer = optim.Adam(params=all_params, lr=config.lr, betas=[0.9, 0.99])
scheduler = torch.optim.lr_scheduler.StepLR(
    optimizer, step_size=config.decay_step_size, gamma=0.1)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch >= args.epochs - config.val_last:
            torch.save(model.state_dict(), os.path.join(
                args.ckpt_dir, 'ep{}.pth'.format(epoch)))
        if config.validation:
            if np.max(np.array(val_measures)[:, 0].squeeze()) == measures[0]:
                best_weights_before = [os.path.join(args.ckpt_dir, weight_file) for weight_file in os.listdir(
                    args.ckpt_dir) if 'best_' in weight_file]
                for best_weight_before in best_weights_before:
                    os.remove(best_weight_before)
                torch.save(model.state_dict(), os.path.join(
                    args.ckpt_dir, 'best_ep{}_Smeasure{:.4f}.pth'.format(epoch, measures[0])))
...
def train(epoch):
    ...
    for batch_idx, (batch, batch_seg) in enumerate(zip(train_loader, train_loader_seg)):
        ...
        scaled_preds = scaled_preds[-min(config.loss_sal_layers+int(
            bool(config.refine)), 4+int(bool(config.refine))):]

        # Tricks
        if config.lambdas_sal_last['triplet']:
            loss_sal, loss_triplet = dsloss(
                scaled_preds, gts, norm_features=norm_features, labels=cls_gts)
        else:
            loss_sal = dsloss(scaled_preds, gts)
        if config.label_smoothing:
            loss_sal = 0.5 * (loss_sal + dsloss(scaled_preds,
                              generate_smoothed_gt(gts)))
        if config.self_supervision:
            H, W = inputs.shape[-2:]
            images_scale = F.interpolate(inputs, size=(
                H//4, W//4), mode='bilinear', align_corners=True)
            sal_scale = model(images_scale)[0][-1]
            atts = scaled_preds[-1]
            sal_s = F.interpolate(atts, size=(H//4, W//4),
                                  mode='bilinear', align_corners=True)
            loss_ss = saliency_structure_consistency(
                sal_scale.sigmoid(), sal_s.sigmoid())
        ...
        if 'cls_mask' in config.loss:
            ...
            for pred_cls_mask in pred_cls_masks:
                loss_cls_mask += F.cross_entropy(pred_cls_mask,
                                                 cls_gts) * config.lambda_cls_mask
            loss += loss_cls_mask
        if config.lambda_adv:
            # gen
            valid = Variable(
                Tensor(scaled_preds[-1].shape[0], 1).fill_(1.0), requires_grad=False)
        ...
        if config.lambda_adv and batch_idx % 5 == 0:
            # disc
            fake = Variable(
                Tensor(scaled_preds[-1].shape[0], 1).fill_(0.0), requires_grad=False)
            optimizer_d.zero_grad()
            adv_loss_real = adv_criterion(disc(gts), valid)
            adv_loss_fake = adv_criterion(
                disc(scaled_preds[-1].detach()), fake)
        ...
        if batch_idx % 20 == 0:
            ...
            if config.lambda_adv:
                info_loss += ', loss_adv: {:.3f}, loss_adv_disc: {:.3f}'.format(
                    adv_loss_g, adv_loss_d)
            if config.lambdas_sal_last['triplet']:
                info_loss += ', loss_triplet: {:.3f}'.format(loss_triplet)
            info_loss += ', Loss_total: {loss.val:.3f} ({loss.avg:.3f})  '.format(
                loss=loss_log)
            logger.info(''.join((info_progress, info_loss)))
    scheduler.step()
    info_loss = '@==Final== Epoch[{0}/{1}]  Train Loss: {loss.avg:.3f}  '.format(
        epoch, args.epochs, loss=loss_log)
    if config.lambdas_sal_last['triplet']:
        info_loss += 'Triplet Loss: {loss.avg:.3f}  '.format(
            loss=loss_log_triplet)
    ...
    for testset in testsets[:1]:
        ...
        for batch in test_loader:
            ...
            os.makedirs(os.path.join(
                saved_root, subpaths[0][0].split('/')[0]), exist_ok=True)
            ...
            for inum in range(num):
                ...
                if config.db_output_refiner or (not config.refine and config.db_output_decoder):
                    res = nn.functional.interpolate(scaled_preds[inum].unsqueeze(
                        0), size=ori_size, mode='bilinear', align_corners=True)
                else:
                    res = nn.functional.interpolate(scaled_preds[inum].unsqueeze(
                        0), size=ori_size, mode='bilinear', align_corners=True).sigmoid()
                save_tensor_img(res, os.path.join(saved_root, subpath))

        eval_loader = EvalDataset(
            saved_root,                                                             # preds
            os.path.join('/gemini/data-1/sod/gts',
                         testset)                     # GT
        )
